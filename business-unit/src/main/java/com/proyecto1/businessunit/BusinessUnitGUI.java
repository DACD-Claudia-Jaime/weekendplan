...
public class BusinessUnitGUI extends JFrame {
    ...
    private JPanel createEventsPanel() {
        ...
        cityComboBox = new JComboBox<>(new String[]{
                "Barcelona", "Madrid", "Valencia", "London", "Berlin",
                "Rome", "Amsterdam", "Lisbon", "Tokyo", "Sydney", "Dubai"
        });
        ...
        originComboBox = new JComboBox<>(new String[]{
                "MAD", "BCN", "LHR", "JFK", "CDG", "FRA", "AMS", "NRT", "SYD", "DXB"
        });
        topPanel.add(originComboBox);
        topPanel.add(new JLabel("Destino:"));
        destinationComboBox = new JComboBox<>(new String[]{
                "MAD", "BCN", "LHR", "JFK", "CDG", "FRA", "AMS", "NRT", "SYD", "DXB"
        });
    ...
    private void searchEvents() {
        ...
        try {
            ...
            Set<String> eventosMostrados = new HashSet<>();
            ...
            for (String evento : eventos) {
                if (evento.toLowerCase().contains(ciudadSeleccionada.toLowerCase())) {
                    ...
                    if (artista.equals("(sin artista)") && nombreEvento.equals("(sin nombre)")
                            && tipo.equals("(sin tipo)") && ciudad.equals("(sin ciudad)") && fecha.equals("(sin fecha)")) {
                        continue;
                    }
                    ...
                    if (!eventosMostrados.contains(textoFormateado)) {
                        eventsTextArea.append(textoFormateado + "\n");
                        eventosMostrados.add(textoFormateado);
                        hayResultados = true;
                    }
                }
            }
    ...
    private void searchFlights() {
        ...
        try {
            ...
            Set<String> vuelosMostrados = new HashSet<>();
            boolean hayResultados = false;
            for (String vuelo : vuelos) {
                if (vuelo.contains(origen + " -> " + destino) && !vuelosMostrados.contains(vuelo)) {
                    flightsTextArea.append(vuelo + "\n");
                    vuelosMostrados.add(vuelo);
                    hayResultados = true;
                }
            } if (!hayResultados) {
